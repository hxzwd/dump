...
/*		if(foff != 0 && !flag_once)
		{
			...
			rewind(file_length);
			tbuf = malloc(sizeof(char)*(file_length + 1));
			if(tbuf == NULL)
			{
				perror("tbuf malloc error");
				fclose(fd);
				exit(1);
			}
			tmp_int = fread(tbuf, sizeof(char), file_length, fd);
			rewind(fd);
			if(tmp_int <= 0)
			{
				perror("fread in tbuf error, tmp_int <= 0");
				fclose(fd);
				exit(1);
			}
			flag_once = 1;
		...
		fgetpos(fd, &cpos);
		printf("current pos in file is %d\n", cpos);
//		fsetpos(fd, &foff);
...
/*			if(tbuf != NULL)
			{
				int tmp_int = atoi(fileoff);
				printf("tbuf is not empty: %s\n", tbuf);
				free(tbuf);
				tbuf = NULL;
			} */
		}
